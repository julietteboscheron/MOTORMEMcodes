%% What does this do?
% Preproc_baseline_epoch is used to epoch the baseline block with motor
% imagery & actual movement which we had participants do at the end of the
% retrieval part. 
...
for isubj = 1:length(subj_list)
    ...
    for i=1:length(block_list)
        ...
        load(['../Data_/DataProcessed_/s' num2str(subj) '/dataft_process_' str_rectif exp_session '_blockbaseline.mat']);
        load(['../Data_/DataProcessed_/s' num2str(subj) '/triggers_' exp_session '_block_baseline.mat']);
    ...
    if exist('data_epoch', 'var')
        save(['../Data_/DataProcessed_/s' num2str(subj) '/dataft_baseline_maxrightleft_epochs_' str_rectif str_bsl exp_session '_' cfg_input.epoch_label '.mat'], 'data_epoch', '-v7.3');
    end
end
